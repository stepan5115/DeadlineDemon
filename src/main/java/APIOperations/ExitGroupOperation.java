...
public class ExitGroupOperation extends BotSynchronizedOperation {
    ...
    protected ExitGroupOperation(String username, String password, String groupName,
            ...
            try {
                ...
                if (group.isPresent()) {
                    if (!user.get().getGroups().contains(group.get().getName())) {
                        result = "WRONG: Нет подписки на группу";
                    } else {
                        user.get().getGroups().remove(group.get().getName());
