...
public class IncludeSubjectOperation extends BotSynchronizedOperation {
    ...
    protected IncludeSubjectOperation(String username, String password,
            ...
            if (subject.isPresent()) {
                try {
                    if (user.get().getNotificationExcludedSubjects().contains(subjectId)) {
                        user.get().getNotificationExcludedSubjects().remove(subjectId);
                        userRepository.save(user.get());
                        result = "OK";
                        updateAuthorizedUsersStatus(user.get().getUser_id());
                    }
                    else
                        result = "WRONG: Дисциплина не исключена";
                } catch (Throwable e) {
    ...
    protected void updateAuthorizedUsersStatus(Long accountId) {
        for (User user : usersProvider.getAuthorizedUsers().values()) {
            if (user.getUser_id().equals(accountId))
                user.getNotificationExcludedSubjects().remove(subjectId);
        }
    }
}